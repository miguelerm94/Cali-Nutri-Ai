...
**Estado:** ✅ APROBADO — FIX-02, FIX-08 aplicados
...
|Fix       |Descripción                                                                                                                                                                                                           |
|----------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|**FIX-02**|`daily_summary_mv` con RLS habilitado. Las MV no heredan RLS de tablas fuente. Sin esta policy, el rol `authenticated` podía leer datos de todos los usuarios via REST API directa de Supabase                        |
|**FIX-08**|`notification_logs` — decisión de escritura documentada explícitamente. Sin `UPDATE` policy para `authenticated`. `opened_at` actualizado exclusivamente via `service_role` a través de `POST /notifications/:id/open`|
...
|-------------------------|---------------------------------------------------------------------------------|
|**Cobertura total**      |RLS habilitado en las 19 tablas de usuario + MV                                  |
|**Catálogos públicos**   |`exercises`, `foods` → solo lectura para `authenticated` con `is_active=TRUE`    |
|**Datos de usuario**     |Acceso solo al propietario via `auth.uid()`                                      |
|**Acceso indirecto**     |`workout_days`, `workout_exercises`, `workout_logs`, `ai_messages` → EXISTS chain|
|**Backend bypass**       |`service_role` → bypass automático de RLS en Supabase                            |
|**Vistas materializadas**|RLS explícito requerido (no heredan de tablas fuente)                            |
...
|-------------------|-----------------------------------|--------------|-----------------------------------|--------------|
|`users`            |`auth.uid()` + `deleted_at IS NULL`|`service_role`|`auth.uid()` + `deleted_at IS NULL`|`service_role`|
|`user_assessments` |`user_id = auth.uid()`             |`auth.uid()`  |`auth.uid()`                       |`auth.uid()`  |
|`body_measurements`|`user_id = auth.uid()`             |`auth.uid()`  |`auth.uid()`                       |`auth.uid()`  |
|`goals`            |`user_id = auth.uid()`             |`auth.uid()`  |`auth.uid()`                       |`auth.uid()`  |
|`training_programs`|`user_id = auth.uid()`             |`auth.uid()`  |`auth.uid()`                       |`auth.uid()`  |
|`workout_days`     |EXISTS → `tp.user_id = auth.uid()` |EXISTS        |EXISTS                             |EXISTS        |
|`exercises`        |`authenticated AND is_active=TRUE` |`service_role`|`service_role`                     |`service_role`|
|`workout_exercises`|EXISTS chain                       |EXISTS chain  |EXISTS chain                       |EXISTS chain  |
|`workout_sessions` |`user_id = auth.uid()`             |`auth.uid()`  |`auth.uid()`                       |`auth.uid()`  |
|`workout_logs`     |EXISTS → `ws.user_id = auth.uid()` |EXISTS        |EXISTS                             |EXISTS        |
|`foods`            |`authenticated AND is_active=TRUE` |`service_role`|`service_role`                     |`service_role`|
|`food_diary`       |`user_id = auth.uid()`             |`auth.uid()`  |`auth.uid()`                       |`auth.uid()`  |
|`water_logs`       |`user_id = auth.uid()`             |`auth.uid()`  |`auth.uid()`                       |`auth.uid()`  |
|`health_data`      |`user_id = auth.uid()`             |`auth.uid()`  |`auth.uid()`                       |`auth.uid()`  |
|`ai_conversations` |`user_id = auth.uid()`             |`auth.uid()`  |`auth.uid()`                       |`auth.uid()`  |
|`ai_messages`      |EXISTS → `ac.user_id = auth.uid()` |EXISTS        |EXISTS                             |EXISTS        |
|`subscriptions`    |`user_id = auth.uid()`             |`service_role`|`service_role`                     |`service_role`|
|`notification_logs`|`user_id = auth.uid()`             |`service_role`|`service_role` *(FIX-08)*          |`service_role`|
|`sync_queue_items` |`user_id = auth.uid()`             |`auth.uid()`  |`auth.uid()`                       |`auth.uid()`  |
|`daily_summary_mv` |`user_id = auth.uid()` *(FIX-02)*  |N/A           |N/A                                |N/A           |

-----

*02_rls_policies_v2.md · CALI-NUTRI AI · Arquitectura Principal · Junio 2026*
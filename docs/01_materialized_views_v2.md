...
**Ejecutar:** DESPUÉS de `prisma migrate deploy` · ANTES de `02_rls_policies.sql`  
**Estado:** ✅ APROBADO — FIX-01, FIX-03, FIX-04, FIX-06, FIX-07 aplicados
...
|Fix       |Descripción                                                                                                                                                            |
|----------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|**FIX-01**|`idx_users_onboarding_incomplete` conservado como índice PARCIAL. Nombre distinto al índice Prisma `idx_users_onboarding_status`. Sin conflicto en migración           |
|**FIX-03**|`daily_summary_mv` reescrita con CTE `activity_dates` — `UNION` de `food_diary + water_logs + health_data`. Incluye días con solo agua o pasos (sin entradas de comida)|
|**FIX-04**|Dos `UNIQUE PARTIAL INDEX` nuevos: `idx_goals_one_active_per_user` y `idx_user_assessments_one_active_per_user`. Garantía de unicidad activa a nivel de DB             |
|**FIX-06**|`get_user_profile()` refactorizada con CTEs. Subquery `user_assessments` ejecutada 1 vez (antes: 3x)                                                                   |
|**FIX-07**|Nuevo índice `idx_workout_sessions_program_date` para `StagnationDetectorEngine` (FD-02)                                                                               |
...
## Mapa de Índices por Tabla

|Tabla              |Índice                                    |Tipo                                                         |Uso                     |
|-------------------|------------------------------------------|-------------------------------------------------------------|------------------------|
|`users`            |`idx_users_onboarding_incomplete`         |Parcial `WHERE onboarding_complete=FALSE`                    |State machine recovery  |
|`users`            |`idx_users_not_deleted`                   |Parcial `WHERE deleted_at IS NULL`                           |Filtro soft delete      |
|`goals`            |`idx_goals_active`                        |Parcial `WHERE status='active'`                              |Hot path dashboard      |
|`goals`            |`idx_goals_one_active_per_user`           |UNIQUE Parcial `WHERE status='active'`                       |Integridad activo único |
|`training_programs`|`idx_training_programs_active`            |Parcial `WHERE status='active'`                              |Dashboard + PremiumGuard|
|`user_assessments` |`idx_user_assessments_active`             |Parcial `WHERE is_active=TRUE`                               |Nivel del usuario       |
|`user_assessments` |`idx_user_assessments_one_active_per_user`|UNIQUE Parcial `WHERE is_active=TRUE`                        |Integridad activo único |
|`subscriptions`    |`idx_subscriptions_active`                |Parcial `WHERE status IN ('active','trial')`                 |PremiumGuard hot path   |
|`workout_sessions` |`idx_workout_sessions_program_date`       |`(program_id, started_at DESC) WHERE finished_at IS NOT NULL`|StagnationDetectorEngine|
|`foods`            |`idx_foods_name_trgm`                     |GIN pg_trgm                                                  |Búsqueda < 500ms        |
|`foods`            |`idx_foods_name_es_trgm`                  |GIN pg_trgm                                                  |Búsqueda español        |
|`foods`            |`idx_foods_name_fts_es`                   |GIN tsvector                                                 |Full-text español       |
|`daily_summary_mv` |`idx_daily_summary_mv_user_date`          |UNIQUE — requerido CONCURRENTLY                              |Refresh sin bloqueo     |
|`daily_summary_mv` |`idx_daily_summary_mv_user_date_range`    |`(user_id, date DESC)`                                       |Analítica histórica     |

-----

*01_materialized_views_v2.md · CALI-NUTRI AI · Arquitectura Principal · Junio 2026*
...
**Estado:** ✅ APROBADO — FIX-05 aplicado (archivo nuevo)

-----

## Propósito

Prisma 5.x no soporta `CHECK` constraints nativamente en `schema.prisma`. Este archivo implementa validaciones de rango y lógica de negocio directamente en la base de datos, como tercera capa de defensa tras el cliente (React Native) y el backend (NestJS Zod DTOs).

### Estrategia de validación en tres capas

|Capa             |Herramienta                             |Alcance                                  |
|-----------------|----------------------------------------|-----------------------------------------|
|**Cliente**      |React Native — validación en formularios|UX, prevención proactiva                 |
|**Backend**      |NestJS Zod DTOs — validación de request |API, rechazo de input inválido           |
|**Base de datos**|`CHECK` constraints (este archivo)      |Garantía definitiva, inmune a bugs de app|

### Por qué es necesaria esta capa

Los constraints de DB protegen contra escenarios que la app-layer no puede cubrir completamente: errores de lógica en servicios, scripts de migración de datos, operaciones batch via `service_role`, race conditions en operaciones concurrentes, y llamadas directas al Supabase REST API.

-----

## Resumen de Constraints

|Tabla              |Constraints|Campos cubiertos                                                                       |
|-------------------|-----------|---------------------------------------------------------------------------------------|
|`users`            |5          |`training_frequency`, `onboarding_step`, `streak_days`, `height_cm`, `target_weight_kg`|
|`user_assessments` |3          |Scores individuales 0–100, `global_score` 0–100, reps no negativos                     |
|`body_measurements`|6          |`weight_kg`, `waist_cm`, `neck_cm`, `hip_cm`, `body_fat_percent`, `bmi`, `lean_mass_kg`|
|`exercises`        |1          |`difficulty` 1–10                                                                      |
|`workout_exercises`|4          |`sets`, `reps_target`, `rest_seconds`, `target_rpe`                                    |
|`workout_sessions` |3          |`subjective_fatigue`, `duration_minutes`, `finished_at >= started_at`                  |
|`workout_logs`     |3          |`set_number`, `reps_completed`, `rpe`                                                  |
|`food_diary`       |3          |`quantity_g`, macros no negativos, `calories` límite                                   |
|`water_logs`       |2          |`amount_ml > 0`, `amount_ml <= 5000`                                                   |
|`goals`            |4          |`target_calories`, `tdee`, macros, `target_date > start_date`                          |
|`training_programs`|1          |`weekly_frequency` 1–7                                                                 |
|`health_data`      |4          |`steps`, `weight_kg`, `sleep_minutes`, `active_calories`                               |
|`subscriptions`    |1          |`current_period_end > current_period_start`                                            |
|`sync_queue_items` |2          |`retry_count >= 0`, `retry_count <= 10`                                                |
|**Total**          |**47**     |                                                                                       |
...
## Justificación de Rangos por Dominio

### Fisiológicos

|Campo             |Rango    |Justificación                                                |
|------------------|---------|-------------------------------------------------------------|
|`height_cm`       |50–272   |50cm = recién nacido. 272cm = record mundial                 |
|`weight_kg`       |1–700    |1kg = edge case clínico. 700kg = máximo documentado          |
|`body_fat_percent`|3–70     |3% = élite competición (mínimo viable). 70% = obesidad severa|
|`bmi`             |8–80     |8 = desnutrición extrema. 80 = obesidad extrema              |
|`sleep_minutes`   |0–1440   |0 = sin sueño registrado. 1440 = 24h                         |
|`steps`           |0–100.000|100.000 pasos ≈ 80km — máximo realista diario                |

### De negocio (CALI-NUTRI AI)

|Campo                    |Rango     |Justificación FinalDecisions                                 |
|-------------------------|----------|-------------------------------------------------------------|
|`training_frequency`     |0–7       |FD-05: Mapeo a factor TDEE (sedentario→doble sesión)         |
|`onboarding_step`        |0–7       |FD-ARCH-07: State machine de 8 pasos                         |
|`difficulty` (ejercicios)|1–10      |FD-04: Escala calistenia (rodillas→planche press)            |
|`rpe` (logs)             |1–10      |FD-04: ProgressionEngine ajusta con RPE real                 |
|`subjective_fatigue`     |1–10      |FD-02: Señal conversacional para CALI                        |
|`global_score`           |0–100     |FD-01: 0-54 beginner, 55-74 intermediate, 75-100 advanced    |
|`target_calories`        |500–10.000|FD-06: Mínimo clínico dieta restrictiva — máximo atleta élite|
|`retry_count`            |0–10      |FD-DB-09: Tras 10 fallos → `status='failed'`                 |
|`amount_ml` (agua)       |1–5.000   |FD-07: Máximo 5L por registro individual                     |

-----

*03_check_constraints_v2.md · CALI-NUTRI AI · Arquitectura Principal · Junio 2026*